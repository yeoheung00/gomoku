...
export const initGame = (io: Server) => {
  io.on("connection", (socket) => {
    ...
    socket.on(
      "enter-room",
      (data: { gameId: string; userId: string }, callback) => {
    ...
    socket.on("placement", (data: { x: number; y: number; userId: string }) => {
      ...
      game.board[data.y][data.x] =
        data.userId === game.players[BLACK].userId ? BLACK : WHITE;
      const isGameOver = winningCondition(
        game.board,
        data.x,
        data.y,
        game.currentTurn,
    ...
    socket.on("send-chat", (data: ChatLog) => {
      const rooms = Array.from(socket.rooms);
      const roomId = rooms.find((room) => room !== socket.id);
      if (!roomId) return;
      io.to(roomId).emit("update-chat", data);
    });
    ...
    socket.on("disconnect", () => {
      ...
      for (const gameId in games) {
        ...
        if (player) {
          ...
          log(
            "info",
            `Player ${player.userId} disconnected from room ${gameId}`,
          );
          break;
        }
      }
